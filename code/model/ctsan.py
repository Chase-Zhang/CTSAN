...
from model import recons_video,flow_pwc
from utils import utils
from model.tsa import TSAFusion
...
def make_model(args):
    ...
    recons_pretrain_fn = ''
    is_mask_filter = True
    return CTSAN(in_channels=args.n_colors, n_sequence=args.n_sequence, out_channels=args.n_colors,
...
class CTSAN(nn.Module):
    ...
    def __init__(self, in_channels=3, n_sequence=3, out_channels=3, n_resblock=3, n_feat=32,
        ...
        self.recons_net = recons_video.RECONS_VIDEO(in_channels=1, n_sequence=3, out_channels=out_channels,
                                                    n_resblock=n_resblock, n_feat=n_feat, extra_channels=extra_channels,n_in_feat=n_feat+1)
        if load_recons_net: 
        ...
        # TSA
        self.conv_tsa = nn.Conv2d(3, n_feat, 3, 1, 1)
        self.tsa_fusion = TSAFusion(num_feat=n_feat, num_frame=3, center_frame_idx=1)
    ...
    def forward(self, x):
        ...
        # TSA
        fea = torch.stack([warped01, frame_list[1], warped21], dim=1)  # 8, 3, 3, 256, 256
        conv_fea = self.conv_tsa(fea.view(-1, c, h, w))
        tsa_align = self.tsa_fusion(conv_fea.view(b, -1, 32, h, w))
        concated = torch.cat([tsa_align, luckiness], dim=1)
        ...
        luckiness = self.get_masks(frame_warp_list, flow_mask_list)
        # TSA
        fea = torch.stack([warped12, frame_list[2], warped32], dim=1)  # 8, 3, 3, 256, 256
        conv_fea = self.conv_tsa(fea.view(-1, c, h, w))
        tsa_align = self.tsa_fusion(conv_fea.view(b, -1, 32, h, w))
        concated = torch.cat([tsa_align, luckiness], dim=1)
        ...
        luckiness = self.get_masks(frame_warp_list, flow_mask_list)
        fea = torch.stack([warped23, frame_list[3], warped43], dim=1)  # 8, 3, 3, 256, 256
        conv_fea = self.conv_tsa(fea.view(-1, c, h, w))
        tsa_align = self.tsa_fusion(conv_fea.view(b, -1, 32, h, w))
        concated = torch.cat([tsa_align, luckiness], dim=1)
        ...
        luckiness = self.get_masks(frame_warp_list, flow_mask_list)
        fea = torch.stack([warped_recons12, recons_2, warped_recons32], dim=1)  # 8, 3, 3, 256, 256
        conv_fea = self.conv_tsa(fea.view(-1, c, h, w))
        tsa_align = self.tsa_fusion(conv_fea.view(b, -1, 32, h, w))
        concated = torch.cat([tsa_align, luckiness], dim=1)
