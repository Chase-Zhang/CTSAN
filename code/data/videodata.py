...
class VIDEODATA(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs = np.array(inputs_list) # (5,256,256,3)
        gts = np.array(gts_list) # (5,256,256,3)
    ...
    def get_patch(self, input, gt, size_must_mode=1):
        if self.train:
            input, gt = utils.get_patch(input, gt, patch_size=self.args.patch_size) # give a limited size but the location is random.(zc)
            h, w, c = input.shape
